...
const nextConfig = {
  ...
  async headers() {
    return [
      {
        // Point agents at the machine-readable entrypoints (RFC 8288). These
        // ride on the homepage because that is where discovery starts.
        source: "/",
        headers: [
          {
            key: "Link",
            value: [
              ...
              '</r/registry.json>; rel="service-desc"; type="application/json"',
      ...
      {
        // Registry JSON is regenerated by CI whenever a component changes, so
        // it must never be `immutable` — clients would pin a stale component
        // forever. Cache at the edge instead and revalidate in the background.
        source: "/r/:path*",
        headers: [
          {
            key: "Cache-Control",
  ...
  async rewrites() {
    return [
      // Serve the raw Markdown of any docs page by appending `.md`, per the
